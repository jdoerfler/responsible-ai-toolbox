...
class MNISTC(_MNIST_C):
    """
    A MNIST-based test dataset for benchmarking neural network robustness to common
    ...
    References
    ----------
    .. [1] https://zenodo.org/record/3239543#.Yp-A6KjMKUk
    """

    url = "https://zenodo.org/record/3239543/files/mnist_c.zip?download=1"
    filename = "mnist_c.zip"
    ## likely don't need such an argument as below for zip
    # tar_md5: str = "56bf5dcef84df0e2308c6dcbcbbd8499"

    ## potentially unnecessary because the root below will have this base folder
    # base_folder: str = "mnist_c"

    ## this needs replaced, potentially with a dict of the paths for each corruption
    ...
    # could be where the mnist_c_leftovers' corruptions go
    extra_corruptions = (
    )
    ...
    def __init__(
        ...
        ----------
        root : PathLike
            Root directory of dataset where directory can access
            ``\zigzag\test_images.npy`` or will be saved to if download is set to True.

        corruption : str
            The type of corruption, e.g., "canny_edges". See `MNISTC.all_corruptions()` for a full list of corruptions.
        ...
        self.corruption_dir = (Path(root) / self.corruption).resolve()
        ...
        # need to figure out what mmap does 
        # currently this only takes the training images
        mmap = np.load(self.corruption_dir / ( 'train_images' + ".npy"), mmap_mode="r")
        size = len(mmap) // 5

        # shape-(N, H, W, C)
        self.data = np.array(mmap[severity * size : (severity + 1) * size])
        self.targets = np.load(self.corruption_dir / "train_labels.npy")

    def _check_integrity(self) -> bool:
        img_path = self.corruption_dir / ('train_images' + ".npy")
        labels_path = self.corruption_dir / "train_labels.npy"
    ...
    def download(self) -> None:
        if self._check_integrity():
            ...
            return
        # last argument here needs swapped with unzip and 
        # need to fix this function to check for zips
...
# the equivalent of this extentsion of CIFAR dataset could be 
# mnist-c leftovers, which has another ~15 types of corruptions
